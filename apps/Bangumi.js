...
async function getItems () {
    ...
    const datanow = new Date(); // 使用 datanow 替换 now
    const month = String(datanow.getMonth() + 1).padStart(2, '0'); // 月份转换为两位数字
    const day = String(datanow.getDate()).padStart(2, '0'); // 日期转换为两位数字
    const key = `itemDetails_${month}${day}`;

    await redis.set(key, JSON.stringify(itemDetails));
...
async function test() {
    ...
    let itemDetailsJson = await redis.get(key);
    if (Buffer.isBuffer(itemDetailsJson)) {
        itemDetailsJson = itemDetailsJson.toString();
    }
